...
exports.courseRouter.get('/', middleware_1.requireAuth, async (req, res) => {
    const courses = await db_1.prisma.course.findMany({ include: { student_relationships: true } });
...
exports.courseRouter.patch('/:courseCode/enroll', middleware_1.requireAuth, async (req, res) => {
    ...
    const course = await db_1.prisma.course.findUnique({ where: { code: courseCode } });
    if (!course) {
